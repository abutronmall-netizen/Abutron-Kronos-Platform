...
import {Bootstrap,Broker,Notice,bootstrap as loadBootstrap,brokers as loadBrokers,createAccount,device,login,notices,readNotice,register} from "./src/api";
...
export default function App(){
 const[token,setToken]=useState<string|null>(null),[data,setData]=useState<Bootstrap|null>(null),[alerts,setAlerts]=useState<Notice[]>([]),[brokerList,setBrokerList]=useState<Broker[]>([]);
 ...
 const[brokerId,setBrokerId]=useState(""),[brokerLogin,setBrokerLogin]=useState(""),[serverName,setServerName]=useState("");
 ...
 async function refresh(active=token){if(!active)return;setRefreshing(true);try{
  const [b,n,bs]=await Promise.all([loadBootstrap(active),notices(active),loadBrokers(active)]);setData(b);setAlerts(n);setBrokerList(bs);if(!brokerId&&bs[0])setBrokerId(bs[0].id);
 ...
 async function linkAccount(){
  if(!token||!brokerId||!brokerLogin.trim())return Alert.alert("Account","Choose a broker and enter the MT5 login.");
  setBusy(true);try{await createAccount(token,brokerId,brokerLogin.trim(),serverName.trim());setBrokerLogin("");setServerName("");await refresh(token)}
  catch(e){Alert.alert("Account link failed",e instanceof Error?e.message:String(e))}finally{setBusy(false)}
 }
 ...
 {screen==="accounts"&&<><Card><Text style={s.cardTitle}>Link trading account</Text><Text style={s.muted}>Select your broker, then enter the MT5 login and server.</Text><View style={s.chips}>{brokerList.map(b=><Pressable key={b.id} onPress={()=>setBrokerId(b.id)} style={[s.chip,brokerId===b.id&&s.chipActive]}><Text style={s.chipText}>{b.display_name}</Text></Pressable>)}</View><TextInput style={s.input} placeholder="MT5 login" placeholderTextColor="#697386" value={brokerLogin} onChangeText={setBrokerLogin}/><TextInput style={s.input} placeholder="Server name" placeholderTextColor="#697386" value={serverName} onChangeText={setServerName}/><Button title="Link account" onPress={linkAccount}/></Card>{accounts.length===0&&<Text style={s.muted}>No trading accounts registered.</Text>}{accounts.map(a=><Card key={a.id}><Text style={s.cardTitle}>{a.broker_login}</Text><Text style={s.row}>{"Server: "+(a.server_name||"-")}</Text><Text style={s.row}>{"Equity: $"+String(a.equity_usd)}</Text><Text style={s.row}>{"Assigned bot: "+a.bot_tier}</Text><Text style={s.row}>{"Route: "+a.route_reason}</Text><Text style={s.row}>{"Status: "+a.status}</Text></Card>)}{licenses.map(l=><Card key={l.id}><Text style={s.cardTitle}>{l.product+" license"}</Text><Text style={s.row}>{"Status: "+l.status}</Text></Card>)}</>}
